...
def inicio():
    ...
    print(
        f"La palabra comienza con la letra {palabra[0]} y termina con la letra {palabra[-1]}")
...
# los parámetros son la letra que arriesga el usuario, la palabra a adinvinar, la respuesta que tiene al momento y las vidas restantes
def verificarLetra(letra, palabra, respuesta, vidas):
   # flag=False          #puse "flag" pero es solo un indicador de que cambia cuando acierta o queda en false hata tango gane
    if letra not in palabra:
        ...
        print(
            "La letra ingresada no pertenece a la palabra, le restan {} vidas".format(vidas))
...
def main(palabra):
    ...
    for letras in palabra:
        respuesta.append(" _")
    # con esta variable determinamos si adivinó (pasamos a True el segundo elemento), la cantidad de vidas que le quedan y las letras que va a adivinando
    verificacion = (respuesta, False, vidas)
    # and verificacion[2]>0:   #mientras no haya adivinado (false) y tenga mas de "0" vidas....
    while verificacion[1] == False:
    ...
    if verificacion[1] == True:
        print("*************************************")
        print('Correcto, adívinó la palabra "{}" y finalizó con  {} corazones'.format(
            "".join(palabra), verificacion[2]))
        print("*************************************")
        nuevoJuego = int(
            input("Desea: \n Jugar de nuevo= (1) \n Salir= (2) \n"))
        if nuevoJuego == 1:
